...
public class Day07_05_BookApplication { // c s
	...
	static Member[] members = new Member[100]; // 모든 클래스에서 사용하는 회원목록
	...
	void menu() {
		while(true) { // 무한루트 [ 종료조건 없음 ]
			...
			if( result == null ) {
				...
				// 1. 일반회원 메뉴 메소드 호출
				System.out.println(" 알림)) 안녕하세요~ " +result+"님");
				membermenu( result ); // 인수 1개 [ 로그인 성공한 id ]
			  }
